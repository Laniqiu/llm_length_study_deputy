...
Usage: change scaffold and out_root as needed
  # If scaffold is baseline, lengths is always 1
  python run_length.py \
    --scaffold baseline \
    --lengths 1 \
    --in_dev data/dev.jsonl \
    --in_enriched data/boolq_enriched.jsonl \
    --out_root runs/phi3 \
    --id_include dev_0021-dev_0022 --skip_existing \
  ...
  python run_length.py \
    ...
    --in_enriched data/boolq_enriched.jsonl \
    --out_root runs/phi3 \
    --id_include dev_0014-dev_0020 --skip_existing \
    --model microsoft/Phi-3-mini-4k-instruct \
    --device mps --dtype float16 \
    --max_new_tokens_tasks 64 --max_new_tokens_final 96 \
    --temperature 0 --stop-seq "### User" \
...
# Example to run only a few specific IDs:
    --id_include dev_0003-dev_0020 --skip_existing \
# or a larger range:
    --num 100 \
"""
...
def main():
    ...
    ap.add_argument("--in_dev", required=True)
    ap.add_argument("--in_enriched", required=True)
    ...
    if args.id_include:
        ...
        logging.info("[FILTER] id_include -> {} items".format(len(items)))
    ...
    logging.info(f"Loaded {len(dev)} dev items, {len(enr)} enriched items")
    logging.info(f"After joining: {len(joined)} items")
    logging.info(f"After filtering: {len(items)} items")
    if items:
        first_item = items[0]
        logging.info(f"First item ID: {first_item[1].get('id', 'unknown')}")
    
    # Load model
    logging.info("Loading model {}.... ".format(args.model))
    runner = ModelRunner(args.model, device=args.device, dtype=args.dtype)
    logging.info("Model loaded on {}, ({})".format(args.device, args.dtype))
    ...
    for i, (drow, erow) in enumerate(tqdm(items, desc="Item: ")):
        ...
        for L in lengths:
            logging.info("Length: {}".format(L))
            # Output path
            fname = f"{boolq_id}_len_L{L}_{args.scaffold}.json"
            out_path = out_root / fname
            if args.skip_existing and out_path.exists():
                logging.info("skip {}...".format(boolq_id))
            ...
            t_start = time.time()
            cur = 0 #! lani
            for t in tqdm(range(1, L + 1), desc="Turn/Length: " ):
                entry = turns[t-1]
                ...
                breakpoint()
                cur += full_prompt.count(" ")
                cur += reply.count(" ")
            logging.debug("Total word count: {}\n".format(cur))
            ...
            logging.info(f"[LEN] id={boolq_id} L={L} {args.scaffold} -> {fname} ({elapsed:.2f}s)")

    write_json(out_root / "_manifest_{}.json".format(args.scaffold), manifest)
    logging.info(f"[DONE] Wrote outputs to {out_root.resolve()}")
    logging.info(f"[STATS] items={len(items)} lengths={lengths} scaffold={args.scaffold} model={args.model} device={args.device}")
    if manifest["items"]:
        done = sum(1 for r in manifest["items"] if not r.get("skipped"))
        logging.info(f"[FILES] {done} new, {len(manifest['items']) - done} skipped")
